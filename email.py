...
def get_email():
    folder_path = PATH
    folder_list = sorted([folder for folder in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, folder))], key=int)
    folder_path = os.path.join(folder_path, str(folder_list[-4]))
    ...
    for file in file_list:
        ...
        with open(file_path, 'rb') as f:
            ...
            padding = b'=' * (4 - (len(content_bytes) % 4) % 4)

            # Add padding to the Base64 bytes
            padded_content_bytes = content_bytes + padding

            # Decode the padded Base64 bytes
            decoded_bytes = base64.b64decode(padded_content_bytes)

            # Print or process the decoded bytes as needed
            print(decoded_bytes)
