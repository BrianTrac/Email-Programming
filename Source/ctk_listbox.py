...
class CTkListbox(customtkinter.CTkScrollableFrame):
    ...
    def __init__(self,
        ...
        super().__init__(master, width=width, height=height,
                         fg_color=fg_color, border_width=border_width, **kwargs)
        ...
        self.font = (
            customtkinter.ThemeManager.theme["CTkFont"]["family"], 13) if font == "default" else font
        if justify == "left":
            self.justify = "w"
        elif justify == "right":
            self.justify = "e"
    ...
    def select(self, index):
        ...
        if self.multiple:
            if self.buttons[index] in self.selections:
                ...
                self.after(100, lambda: self.buttons[index].configure(
                    hover=self.hover))
            else:
                self.selections.append(self.buttons[index])
            for i in self.selections:
                i.configure(fg_color=self.select_color, hover=False)
                self.after(100, lambda button=i: button.configure(
                    hover=self.hover))
        else:
            self.selected = self.buttons[index]
            self.buttons[index].configure(
                fg_color=self.select_color, hover=False)
            self.after(100, lambda: self.buttons[index].configure(
                hover=self.hover))
    ...
    def insert(self, index, name, subject, time, folder):
        ...
        self.buttons[index] = ContentFrame(self, index, name, subject, time, folder, callback=self.update_content_frame)
        self.buttons[index].bind('<Button-1>', command=lambda event, num=index: self.select(num))
        self.buttons[index].pack(padx=0, pady=(0, 5), fill="x", expand=True)

    def update_content_frame(self, message, current_folder):
        self.callback(message, current_folder)
